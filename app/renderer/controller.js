...
const ExpressionWatchView =
  require("./expressionWatchView").ExpressionWatchView;
...
$(document).ready(() => {
  if (InkProject.currentProject == null) {
    ...
    // Debug
    if (loadTestInk) {
      var testInk = require("fs").readFileSync(
        path.join(__dirname, "test.ink"),
        "utf8",
      );
...
LiveCompiler.setEvents({
  ...
  errorsAdded: (errors) => {
    for (var i = 0; i < errors.length; i++) {
      var error = errors[i];
      if (
        error.filename == InkProject.currentProject.activeInkFile.relativePath()
      )
  ...
  unexpectedError: (error) => {
    if (error.indexOf("Unhandled Exception") != -1) {
      PlayerView.addTerminatingMessage(
        i18n._("Sorry, the ink compiler crashed ☹"),
        "error",
      );
      PlayerView.addTerminatingMessage(
        i18n._("Here is some diagnostic information:"),
        "error",
      );
      ...
      PlayerView.addTerminatingMessage(
        i18n._("Ink compiler had an unexpected error ☹"),
        "error",
      );
...
ipc.on("project-stats", (event, visible) => {
  LiveCompiler.getStats((statsObj) => {
    ...
    messageLines.push(
      i18n._(
        "Notes: Words should be accurate. Knots include functions. Gathers and diverts may include some implicitly added ones by the compiler, for example in weave. Diverts include END and DONE.",
      ),
    );
...
ipc.on("keyboard-shortcuts", (event, visible) => {
  ...
  messageLines.push("");
  messageLines.push(
    `${i18n._("Find and Replace")}: Ctrl+H ${i18n._("or")} Cmd+H`,
  );
  messageLines.push("");
  messageLines.push(`${i18n._("Find")}: Ctrl+F ${i18n._("or")} Cmd+F`);
  messageLines.push("");
  messageLines.push(
    `${i18n._("Go to Anything")}: Ctrl+P ${i18n._("or")} Cmd+P`,
  );
  messageLines.push("");
  messageLines.push(
    `${i18n._("Toggle Comment")}: Ctrl+/ ${i18n._("or")} Cmd+/`,
  );
  messageLines.push("");
  messageLines.push(
    `${i18n._("Add Multicursor Above")}: Ctrl+Alt+Up ${i18n._("or")} Ctrl+Option+Up`,
  );
  messageLines.push("");
  messageLines.push(
    `${i18n._("Add Multicursor Below")}: Ctrl+Alt+Down ${i18n._("or")} Ctrl+Option+Down`,
  );
  messageLines.push("");
  messageLines.push(
    `${i18n._("Temporarily Fold/Unfold Selection")}: Alt+L ${i18n._("or")} Ctrl+Option+Down`,
  );
  messageLines.push("");
  alert(messageLines.join("\n"));
});
...
EditorView.setEvents({
  ...
  jumpToSymbol: (symbolName, contextPos) => {
    var foundSymbol = InkProject.currentProject.findSymbol(
      symbolName,
      contextPos,
    );
    if (foundSymbol) {
...
ExpressionWatchView.setEvents({
  change: () => {
    LiveCompiler.setEdited();
    $("#player .scrollContainer").css(
      "top",
      ExpressionWatchView.totalHeight() + "px",
    );
  },
});

ToolbarView.setEvents({
  toggleSidebar: (id, buttonId) => {
...
NavView.setEvents({
  ...
  addInclude: (filename, addToMainInk) => {
    ...
    var newInkFile = InkProject.currentProject.addNewInclude(
      filename,
      addToMainInk,
    );
    if (newInkFile) {
...
ipc.on("set-tags-visible", (event, visible) => {
  if (visible) $("#main").removeClass("hideTags");
  else $("#main").addClass("hideTags");
});
