...
define("ace/ext/menu_tools/generate_settings_menu", [
  ...
  var addEditorMenuOptions =
    require("./add_editor_menu_options").addEditorMenuOptions;
  ...
  module.exports.generateSettingsMenu = function generateSettingsMenu(editor) {
    ...
    function createNewEntry(obj, clss, item, val) {
      ...
      var label = item
        ...
        .replace(/([A-Z])/g, " $1")
        .trim();
      div.appendChild(egen.createLabel(t(label), item));

      if (Array.isArray(val)) {
        val = val.map(function (opt) {
          var translated = {
            textContent: t(opt.textContent),
            value: opt.value,
          };
          if (opt.selected) translated.selected = opt.selected;
          return translated;
        });
...
define("ace/ext/settings_menu", [
  ...
  "use strict";
  var generateSettingsMenu =
    require("./menu_tools/generate_settings_menu").generateSettingsMenu;
