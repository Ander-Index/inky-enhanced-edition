...
ipcMain.handle("try-close", async (event) => {
  return dialog.showMessageBox({
    ...
    buttons: [i18n._("Save"), i18n._("Don't save"), i18n._("Cancel")],
    defaultId: 0,
  });
});
...
app.on("ready", function () {
  ...
  AppMenus.setCallbacks({
    ...
    showSettings: () => {
      var win = ProjectWindow.focused();
      if (win) win.browserWindow.webContents.send("show-settings");
    },
    stats: () => {
      var win = ProjectWindow.focused();
      if (win) win.stats();
    ...
    toggleAnimation: () => {
      ...
      for (let i = 0; i < ProjectWindow.all().length; i++) {
        ...
        eachWindow.browserWindow.webContents.send(
    ...
    toggleAutoComplete: () => {
      let autoCompleteDisabled =
        !ProjectWindow.getViewSettings().autoCompleteDisabled;
      ProjectWindow.addOrChangeViewSetting(
  ...
  AppMenus.setAnimationEnabled(
    ProjectWindow.getViewSettings().animationEnabled,
  );
  AppMenus.setAutoCompleteDisabled(
    ProjectWindow.getViewSettings().autoCompleteDisabled,
  );
