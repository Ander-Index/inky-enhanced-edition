...
const viewSettingsPath = path.join(
  electron.app.getPath("userData"),
  "view-settings.json",
);
...
function ProjectWindow(filePath) {
  const getThemeFromMenu = () =>
    Menu.getApplicationMenu()
      .items.find((e) => e.id === "view")
      .submenu.items.find((e) => e.id === "theme")
  ...
  // New project, new settings
    ...
    this.settings = {};
    if (events.onProjectSettingsChanged) events.onProjectSettingsChanged({});
  }
  ...
  fs.stat(settingsPath, (err, stats) => {
    ...
    fs.readFile(settingsPath, "utf8", (err, fileContent) => {
      if (err) {
        completeSettings(
          {},
          "File read error - failed to load project settings file at: " +
            settingsPath,
        );
        return;
      }
      if (!fileContent) {
        completeSettings(
          {},
          "Project settings file appeared to be empty: " + settingsPath,
        );
        return;
      }
      ...
      try {
        ...
        completeSettings(
          {},
          "Project settings file appeared to be invalid JSON: " +
            settingsPath +
            ": " +
            error,
        );
...
ProjectWindow.focused = function () {
  ...
  if (browWin) return ProjectWindow.withWebContents(browWin.webContents);
  else return null;
};
...
ProjectWindow.withWebContents = function (webContents) {
  ...
  for (var i = 0; i < windows.length; i++) {
    if (windows[i].browserWindow.webContents === webContents) return windows[i];
  }
  return null;
};
...
ProjectWindow.withMainkInkPath = function (absPath) {
  ...
  for (var i = 0; i < windows.length; i++) {
    if (windows[i].mainInkAbsPath == absPath) return windows[i];
  }
  return null;
};
...
function addRecentFile(filePath) {
  ...
  const newRecentFiles =
    recentFiles.indexOf(resolvedFilePath) >= 0
      ? recentFiles
...
ProjectWindow.open = function (filePath) {
  if (!filePath) {
    var multiSelectPaths = dialog.showOpenDialogSync({
      ...
      properties: ["openFile"],
      filters: [{ name: i18n._("Ink files"), extensions: ["ink"] }],
    });
